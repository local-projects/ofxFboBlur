...
void ofxFboBlur::setup( ofFbo::Settings s, bool additive, float scaleDownPercent )
{
    ...
    if( ( !shaderV.isLoaded() && !additive ) || ( additive && !shaderVadd.isLoaded() ) ) {
        ...
        if( ofIsGLProgrammableRenderer() ) { // PROGRAMMABLE PIPELINE //////////////////////////////////////////////
            ...
            // VETEX SHADER /////////////////////////////////////
            vertex = string( "#version 150\n" ) + STRINGIFY( uniform mat4 modelViewProjectionMatrix;\n in vec4 position;\n void main() {
            ...
            // FRAGMENT SHADERs///////////////////////////////////
            fragV = "#version 150\n"
                + (string)STRINGIFY( uniform float blurLevel;\n uniform float gain;\n uniform sampler2DRect src;\n out vec4 fragColor;\n
                                     ...
                                     void main( void ) {
                                         ...
                                         \n
                                             ...
                                             sum
                                             += texture( src, vec2( gl_FragCoord.x, gl_FragCoord.y - 4.0 * blurSize ) ) * 0.049049;
                ...
                + (string)STRINGIFY( uniform float blurLevel;\n uniform float gain;\n uniform sampler2DRect src;\n out vec4 fragColor;\n
                                     ...
                                     void main( void ) {
                                         ...
                                         \n
                                             ...
                                             sum
                                             += texture( src, vec2( gl_FragCoord.x - 4.0 * blurSize, gl_FragCoord.y ) ) * 0.049049;
                                         ...
                                         if( additive )
                                             fragH += STRINGIFY( if( sum.a > 0.0 ) sum.a = 1.0;\n );
            ...
            fragV = "#version 120\n#extension GL_ARB_texture_rectangle : enable\n"
                + (string)STRINGIFY(
                    uniform float blurLevel; uniform float gain; uniform sampler2DRect src;
                    ...
                    void main( void ) {
                        ...
                        sum += texture2DRect( src, vec2( gl_TexCoord[0].x, gl_TexCoord[0].y + 4.0 * blurSize ) ) * 0.049049;
                          ...
                          if( additive )
                              fragV += STRINGIFY( if( sum.a > 0.0 ) sum.a = 1.0; );
...
#else
        blurOutputFBO.getTextureReference().draw(
            0, blurOutputFBO.getHeight(), blurOutputFBO.getWidth() / scaleDown, -blurOutputFBO.getHeight() / scaleDown );
#endif
    }
    ofPopStyle();
}

void ofxFboBlur::drawBlurFbo( float x, float y, float w, float h )
{
    ofPushStyle();
    for( int i = 0; i < numBlurOverlays; i++ ) {
#if( OF_VERSION_MINOR >= 8 )
        blurOutputFBO.getTexture().draw( x, y, w, h );
#else
        blurOutputFBO.getTextureReference().draw(
            0, blurOutputFBO.getHeight(), blurOutputFBO.getWidth() / scaleDown, -blurOutputFBO.getHeight() / scaleDown );
#endif
